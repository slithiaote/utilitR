...
Un avantage de ces deux méthodes est qu'elle manipulent les données directement depuis le dataset Parquet: les données ne sont chargées ni dans la mémoire de `R`, ni dans celle de `DuckDB`. Pour plus de commodité, on sauvegarde l'instruction précédente dans la variable `bpe_ens_2018_dataset`.

```{r}
bpe_ens_2018_ddb <- conn_ddb %>% tbl("read_parquet('bpe_ens_2018_dataset/*.parquet')", check_from = FALSE)
```
...
**Pour écrire une table `duckdb` sur le disque au format Parquet, il est recommandé d'utiliser la librairie `arrow`**: on commence par convertir la table `duckdb` en `Arrow Table`, puis on l'écrit sur le disque en format Parquet.

```{r}
# Convertir la table duckdb en arrow, puis la sauvegarder en Parquet
bpe_ens_2018_ddb %>% 
  arrow::to_arrow() %>% 
  arrow::write_dataset("temp_dataset")
list.files("temp_dataset") # liste des fichiers du répertoire temp_dataset/
```

::: {.callout-tip}
Il est possible d'écrire une table `duckdb` en format Parquet directement avec `duckdb` en utilisant la commande SQL `COPY ... TO`. Toutefois, il est préférable de passer par `arrow` pour deux raisons: 

- Cela permet d'exécuter des calculs plus complexes sans faire exploser la mémoire (car `arrow` sauvegarde au fur et à mesure);
- `arrow` est plus facile à utiliser pour écrire des données en format Parquet, notamment quand on souhaite ajouter des options ou partitionner les données.
:::
...
```{r error=TRUE}
bpe_ens_2018_ddb |> select(DEP) |>
      mutate(NB_EQUIP_TOTAL_DEP  = sum(NB_EQUIP))
```

**Convertir les types**

Dans cet exemple, on veut multiplier un nombre par une indicatrice. Cela provoque une erreur.
...
::: {.callout-tip}
Il est fortement recommandé de lire la fiche [Manipuler des données avec `arrow`](#arrow), notamment la partie sur l'évaluation différée, avant de lire la présente section.
:::

### L'évaluation différée avec `duckdb` (_lazy evaluation_) {#sec-lazy-dbb}

Quand on manipule des objets `duckdb`, on construit des requêtes SQL, mais on ne les exécute pas tant qu'on utilise pas `collect()`, car le _package_ `duckdb` se contente de traduire le code `R` en `SQL` sans l'exécuter. On rappelle qu'il faut utiliser `show_query()` pour visualiser la requête. La fonction `print()` permet de pré-visualiser le résultat.
...
La fonction `collect()` fait exécuter le calcul, et transmet les résultats à `R` sous forme d'un `tibble`. Avant `collect()`, c'est le moteur SQL de `duckdb` qui fait / fera les calculs. Après `collect()`, c'est `R` qui fait le calcul, et l'on manipule un objet `R` (`tibble`) standard, avec toutes les fonctions / _packages_ disponibles.
...
::: {.callout-tip}
Si vous ne savez plus si une table de données est une requête SQL écrite par `duckdb`, un `Arrow Table` ou un `tibble`, il suffit d'exécuter `class(le_nom_de_ma_table)`. 

- Si la table est une requête SQL écrite par `duckdb`, vous obtiendrez ceci: `"tbl_duckdb_connection" "tbl_dbi"               "tbl_sql"  "tbl_lazy"              "tbl" `. 
- Si la table est un `Arrow Table`, vous obtiendrez ceci: `"Table"        "ArrowTabular" "ArrowObject"  "R6"`. 
- Si la table est un `tibble`, vous obtiendrez `"tbl_df"     "tbl"        "data.frame"`.
:::
